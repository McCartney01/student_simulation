...
def chunking_by_token_size(
    content: str, overlap_token_size=128, max_token_size=1024, tiktoken_model="gpt-4o-2024-11-20"
):
...
def chunking_by_chars(
    content: str, chars: str = "-"*10, tiktoken_model="gpt-4o-2024-11-20"
):
    content = content.split(chars)
    results = []
    for i, c in enumerate(content):
        results.append({
            "tokens": len(encode_string_by_tiktoken(c, model_name=tiktoken_model)),
            "content": c.strip(),
            "chunk_order_index": i
        })
    return results
...
async def _handle_single_entity_extraction(
    ...
    if record_attributes[0] != '"entity"' or len(record_attributes) < 3:
        return None
    ...
    entity_master_level = clean_str(record_attributes[2])
    entity_master_description = clean_str(record_attributes[3])
    entity_source_id = chunk_key
    return dict(
        entity_name=entity_name,
        entity_type="Concept",
        entity_master_level=entity_master_level,
        entity_master_description=entity_master_description,
...
async def _handle_single_relationship_extraction(
    ...
    if record_attributes[0] != '"relationship"' or len(record_attributes) < 6:
        return None
...
async def _merge_nodes_then_upsert(
    ...
    description = GRAPH_FIELD_SEP.join(
        sorted(set([dp["entity_master_level"] + ": " + dp["entity_master_description"] for dp in nodes_data] + already_description))
    )
...
async def _merge_edges_then_upsert(
    ...
    if await knwoledge_graph_inst.has_edge(src_id, tgt_id):
        ...
        already_type.append(already_edge["type"])
    ...
    type = GRAPH_FIELD_SEP.join(
        set([dp["type"] for dp in edges_data] + already_type)
    )
...
#         final_result = """("entity"<|>"Glucose"<|>"Glucose is the most common monosaccharide in cells and the main energy substance for cellular respiration.")##
# ("entity"<|>"Monosaccharides"<|>"Monosaccharides are the simplest sugars that cannot be hydrolyzed into smaller sugar molecules. They include glucose, fructose, and galactose.")##
# ("relationship"<|>"Glucose"<|>"Monosaccharides"<|>"Part-of"<|>"Glucose is the most common type of monosaccharide in cells."<|>9)##
# ("relationship"<|>"Monosaccharides"<|>"Glucose"<|>"Contain"<|>"Glucose is the most common type of monosaccharide in cells."<|>9)<|COMPLETE|>
# """
    ...
    for m_nodes, m_edges in results:
        ...
        for k, v in m_edges.items():
            ...
            new_k = '-----'.join(k)
            if new_k not in edge_relation:
                edge_relation[new_k] = {"relation": []}
            edge_relation[new_k]["relation"] += [this_e["type"].replace("\"", "") for this_e in v]
    ...
    return knwoledge_graph_inst, edge_relation
...
async def _find_most_related_text_unit_from_entities(
    ...
    # print("----------------------------alltext_units_lookup")
    # print(all_text_units_lookup)
...
async def _build_local_query_context(
    ...
    if not return_str:
        return {
            "results": results,
            "communities_section_list": communities_section_list,
            "entities_section_list": entites_section_list,
            "relations_section_list": relations_section_list,
            "text_units_section_list": text_units_section_list
        }
    else:
        return f"""
-----Reports-----
...
async def local_query(
    ...
    return_str = False if query_param.only_need_context else True
